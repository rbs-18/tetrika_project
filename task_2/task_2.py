...
def find_next_page(main_block: element.Tag) -> str:
    # TODO нужно брать последнюю ссылку в главном блоке, условие то же

    next_page = main_block.a
    if next_page.get_text(strip=True) == 'Следующая страница':
        next_page = HOST + next_page.get('href')
    else:
        next_page = ''
    print(next_page)
    return next_page
...
def calculate_animals(main_block: element.Tag):
    ...
    for letter_block in letter_blocks:
        ...
        print(letter, animal_count)


def parse(next_page: str):
    while next_page:
        response = get_response(next_page)
        if response.status_code == 200:
            html = response.text
            main_block = find_main_block(html)
            calculate_animals(main_block)
